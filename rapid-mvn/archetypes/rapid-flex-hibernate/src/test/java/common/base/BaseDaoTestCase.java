 ...
 * 
 * @author badqiu 请设置好要装载的spring配置文件,一般开发数据库与测试数据库分开
...
// TestMethodContextExecutionListener用于在@Before时可以得到测试方法名称
public class BaseDaoTestCase
	...
	public void onSetUpInsertTestDatas() throws Exception {
		String jdbcSchema = null; // set schema for
									// oracle,出现AmbiguousTableNameException时，使用命令:purge
	...
	protected void insertTestData(String classpathFileName) {
		try {
			dbUnitHelper.insertTestData(ResourceUtils.getFile("classpath:"
					+ classpathFileName));
		} catch (Exception e) {
			throw new RuntimeException(
					"insertTestData error,classpathFileName:"
							+ classpathFileName, e);
		}
	}
