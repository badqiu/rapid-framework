...
//	        bodyStyle:'padding:10px',
			...
			items:[{
	            	...
	            	<#list table.columns as column>
					<#if column.columnAlias!="id">
	        		,{xtype:'textfield',fieldLabel:'${column.columnAlias}',name:'${column.columnNameLower}',width:288}</#if>
	        		</#list>
	        ]
	    });
...
/**
 * 搜索域类扩展
 * @class ${basepackage}.${classNameLower}.searchfield
 * @extends Ext.app.SearchField
 */
${basepackage}.${classNameLower}.searchfield = Ext.extend(Ext.app.SearchField,{
	initComponent:function() {
		Ext.apply(this,{
	                width: 130
	    });
		${basepackage}.${classNameLower}.searchfield.superclass.initComponent.call(this);
	}
...
${basepackage}.${classNameLower}Grid = Ext.extend(Ext.grid.GridPanel,{
    initComponent:function() {
		...
		Ext.apply(this,{
	        ...
	        , tbar:[
	        	{text:'新增',cls:'x-btn-text-icon',icon:'../../images/add.gif',handler:this.add${className},scope:this},'-'
	        	,{text:'修改',cls:'x-btn-text-icon',icon:'../../images/edit.gif',handler:this.edit${className},scope:this},'-'
	        	,{text:'删除',cls:'x-btn-text-icon',icon:'../../images/delete.gif',handler:this.delete${className},scope:this},'-'
	        	...
	        	},new ${basepackage}.${classNameLower}.searchfield({id:'searchfld',store:this.ds})
	        ]
		});
		//调用父类构建函数
